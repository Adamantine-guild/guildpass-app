...
/**
 * Field-level schema without the cross-field, mode-dependent requirements.
 * Used for plain runtime reads (e.g. `getApiMode()`), where a live-mode
 * deployment missing its secrets should fail at the point the secrets are
 * actually needed (client construction / startup probe), not on every read.
 */
export const dashboardEnvBaseSchema = z
  .object({
    // ── Mode selectors ───────────────────────────────────────────────
...
/**
 * Full dashboard env schema: field-level validation plus the cross-field,
 * mode-dependent requirements documented above. Use this for startup
 * validation and anywhere strict configuration checking is intended.
 */
export const dashboardEnvSchema = dashboardEnvBaseSchema.superRefine((data, ctx) => {
